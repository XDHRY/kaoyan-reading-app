## v5.12.5 — 2026-08-16（输入校验加固 · 持续测试 29 轮）

对应发布：`考研阅读助手 v5.12.5 —— 输入校验加固`

- **纯空格输入校验全闭环**（持续测试第 13~24 轮发现并修复）：
  - `vocab.lookup` 生词查义：纯空格词被拒绝（trim 后空词不入库）
  - 账号体系 5 处 `name`（注册/登录/找回第一步/重置密码/改资料）：纯空格昵称被拒绝
  - AI 出题 `topic` 与作文工坊 6 字段（prompt/note×2/content/title）：纯空格提示词不再进入 LLM
  - 复盘自评 `myReflection`：纯空格不落库
  - 管理/渠道/提示词/诊断 4 路由 13 字段（渠道名/apiKey/模型名/提示词名与内容/诊断内容等）：统一加 trim 校验
